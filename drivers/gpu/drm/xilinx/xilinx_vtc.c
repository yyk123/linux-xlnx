...
/* io write operations */
static inline void xilinx_vtc_writel(void __iomem *base, int offset, u32 val)
{
	writel(val, base + offset);
}

/* io read operations */
static inline u32 xilinx_vtc_readl(void __iomem *base, int offset)
{
				       ...
				       struct xilinx_vtc_polarity *polarity)
{
	...
	xilinx_vtc_writel(vtc->base, VTC_GPOL, reg);
}
			      ...
			      struct xilinx_vtc_hori_offset *hori_offset)
{
	...
	xilinx_vtc_writel(vtc->base, VTC_GVBHOFF_F0, reg);
	...
	reg |= (hori_offset->v0sync_hori_end << VTC_XVXHOX_HEND_SHIFT) &
		VTC_XVXHOX_HEND_MASK;
	xilinx_vtc_writel(vtc->base, VTC_GVSHOFF_F0, reg);
				  ...
				  struct xilinx_vtc_src_config *src_config)
{
	...
	reg = xilinx_vtc_readl(vtc->base, VTC_CTL);
	...
	xilinx_vtc_writel(vtc->base, VTC_CTL, reg);
}
...
void xilinx_vtc_enable(struct xilinx_vtc *vtc)
{
	...
	reg = xilinx_vtc_readl(vtc->base, VTC_CTL);
	xilinx_vtc_writel(vtc->base, VTC_CTL, reg | VTC_CTL_GE);
}
			   ...
			   struct videomode *vm)
{
	...
	reg = xilinx_vtc_readl(vtc->base, VTC_CTL);
	xilinx_vtc_writel(vtc->base, VTC_CTL, reg & ~VTC_CTL_RU);
	...
	reg = htotal & 0x1fff;
	xilinx_vtc_writel(vtc->base, VTC_GHSIZE, reg);

	reg = vtotal & 0x1fff;
	reg |= reg << VTC_GV1_BPSTART_SHIFT;
	xilinx_vtc_writel(vtc->base, VTC_GVSIZE, reg);

	DRM_DEBUG_DRIVER("ht: %d, vt: %d\n", htotal, vtotal);

	reg = hactive & 0x1fff;
	reg |= (vactive & 0x1fff) << 16;
	xilinx_vtc_writel(vtc->base, VTC_GASIZE, reg);
	...
	xilinx_vtc_writel(vtc->base, VTC_GHSYNC, reg);
	...
	xilinx_vtc_writel(vtc->base, VTC_GVSYNC_F0, reg);
	...
	if (vm->flags & DISPLAY_FLAGS_INTERLACED) {
		...
		xilinx_vtc_writel(vtc->base, VTC_GVSYNC_F1, reg);
		reg = xilinx_vtc_readl(vtc->base, VTC_GENC) | VTC_GENC_INTERL;
		xilinx_vtc_writel(vtc->base, VTC_GENC, reg);
	} else {
	...
	pr_info("vtc: VTC Timing: hactive=%d, vactive=%d, hsync=%d, vsync=%d\n",
		vm->hactive, vm->vactive, vm->hsync_len, vm->vsync_len);
}
...
void xilinx_vtc_reset(struct xilinx_vtc *vtc)
{
	...
	xilinx_vtc_writel(vtc->base, VTC_RESET, VTC_RESET_RESET);
...
{
	xilinx_vtc_writel(vtc->base, VTC_IER, VTC_IXR_G_VBLANK |
			  xilinx_vtc_readl(vtc->base, VTC_IER));
}
...
{
	xilinx_vtc_writel(vtc->base, VTC_IER, (intr & VTC_IXR_ALLINTR_MASK) |
			  xilinx_vtc_readl(vtc->base, VTC_IER));
}
...
{
	return xilinx_vtc_readl(vtc->base, VTC_IER) &
	       xilinx_vtc_readl(vtc->base, VTC_ISR) & VTC_IXR_ALLINTR_MASK;
}
...
{
	xilinx_vtc_writel(vtc->base, VTC_ISR, intr & VTC_IXR_ALLINTR_MASK);
}
